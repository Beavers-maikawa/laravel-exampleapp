<x-app-layout>
    ...
    <form action="{{route('recipe.update',['id' => $recipe->id])}}" method="POST" class="container p-4 mx-auto bg-white rounded" enctype="multipart/form-data">
        @csrf
        @method('patch')
        ...
        <div class="grid grid-cols-2 rounded border border-gray-500">
            ...
            <div class="col-span-1 p-4">
                <input type="text" name="title" class="border border-gray-300 rounded p-2 mb-4 w-full" placeholder="レシピ名を入力" value="{{$recipe->title}}">
                <textarea name="description" class="border border-gray-300 p-2 mb-4 w-full rounded" placeholder="レシピの説明を入力">{{$recipe->description}}</textarea>
                ...
                <div id="ingredients" class="mb-4">
                    @foreach ($recipe->ingredients as $i => $ingredient)
                        <div class="flex mb-2 items-center ingredient">
                            ...
                            <input type="text" name="ingredients[{{$i}}][name]" value="{{$ingredient->name}}" class="ingredient-name ml-4 border border-gray-300 p-2 w-1/2 rounded" placeholder="材料名">
                            <p class="mx-2">:</p>
                            <input type="text" name="ingredients[{{$i}}][quantity]" value="{{$ingredient->quantity}}" class="ingredient-quantity border border-gray-300 p-2 w-1/2 rounded" placeholder="分量">
        ...
        <div class="flex justify-center mt-4">
            <button type="submit" class="bg-green-600 text-white p-2 px-4 rounded mb-4 text-right">レシピを更新する</button>
        </div>
        <hr class="my-4">
        ...
        <div id="steps">
            @foreach ($recipe->steps as $i => $s)
                <div class="step flex justify-between items-center mb-2">
                    ...
                    <input type="text" name="steps[]" class="border border-gray-300 p-2 w-full rounded" placeholder="手順を入力" value="{{$s->description}}">
    ...
    <form action="{{route('recipe.destroy',['id' => $recipe->id])}}" method="POST" class="container mx-autorounded">
        @csrf
        @method('delete')
        <button type="submit" id="delete" class="bg-red-600 text-white p-2 px-4 rounded my-4 text-right">レシピを削除する</button>
    </form>
</x-app-layout>